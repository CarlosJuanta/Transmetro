...
const create = async (nombre, id_municipalidad) => {
  const [result] = await pool.execute('INSERT INTO ESTACION (nombre, id_municipalidad) VALUES (?, ?)', [nombre, id_municipalidad]);
  return { id: result.insertId, nombre, id_municipalidad };
};

const getAll = async () => {
  const [estaciones] = await pool.execute(
    `SELECT e.id_estacion, e.nombre, m.id_municipalidad, m.nombre AS nombre_municipalidad, d.nombre AS nombre_departamento
     FROM ESTACION e
...
const update = async (id, nombre, id_municipalidad) => {
  const [result] = await pool.execute('UPDATE ESTACION SET nombre = ?, id_municipalidad = ? WHERE id_estacion = ?', [nombre, id_municipalidad, id]);
