...
const createUsuario = async (req, res) => {
  ...
  try {
    ...
    if (error.code === 'ER_DUP_ENTRY') { return res.status(409).json({ message: 'Ya existe un usuario con ese correo electrónico.' }); }
    if (error.code === 'ER_NO_REFERENCED_ROW_2') { return res.status(404).json({ message: 'El rol o la estación especificados no existen.' }); }
...
const getUsuarioById = async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ message: 'Error al obtener el usuario.' }); }
};
...
const resetPassword = async (req, res) => {
    ...
    try {
        const affectedRows = await usuarioService.updatePassword(id_usuario, nuevaContrasenia);
        if (affectedRows === 0) {
