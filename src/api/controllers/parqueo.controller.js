...
const getAllParqueos = async (req, res) => {
    try {
        const parqueos = await parqueoService.getAll();
        res.status(200).json(parqueos);
    } catch (error) {
        res.status(500).json({ message: 'Error al obtener todos los parqueos.' });
    }
};
...
const createParqueo = async (req, res) => {
  ...
  try {
    ...
    if (error.code === 'ER_NO_REFERENCED_ROW_2') { return res.status(404).json({ message: 'La estación especificada no existe.' }); }
...
const updateParqueo = async (req, res) => {
  ...
  try {
    ...
    if (affectedRows === 0) { return res.status(404).json({ message: 'Parqueo no encontrado.' }); }
...
const deleteParqueo = async (req, res) => {
  ...
  try {
    ...
    if (error.code === 'ER_ROW_IS_REFERENCED_2') { return res.status(409).json({ message: 'No se puede eliminar el parqueo porque tiene buses asignados.' }); }
