...
const generarReporteEstaciones = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error al generar el reporte de estaciones.' });
  }
};
...
const getReporteDistanciaLinea = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Error al generar el reporte de distancia.' });
  }
};
...
const getReporteAccesosPorLinea = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Error al generar el reporte de accesos por línea.' });
  }
};
